...
fn ntcmach_path() -> PathBuf {
    PathBuf::from(r"C:\Users\Administrator\Desktop\制版软件\NTCDLLA5.DLL")
}
...
fn batch_ntcdlla5_dll() {
    ...
    // RM-7: Reconstruct C source from structured IR (all_funcs still borrows built_entries).
    {
        ...
        std::fs::write(r"C:\Users\Administrator\Desktop\ntcdlla5_recovered.c", &src)
            .expect("write recovered.c");
        println!("RM-7 wrote ntcdlla5_recovered.c ({} bytes, {} funcs)", src.len(), cfuncs.len());
    }
