...
fn ntcmach_path() -> PathBuf {
    PathBuf::from(r"C:\Users\Administrator\Doubao\chats\2026-09-08\new-chat-1\fox\rm8_roundtrip.exe")
}
...
fn rm8_roundtrip_decompile() {
    ...
    eprintln!("[RM8] before analyze_binary");
    let result = analyze_binary(&binary).expect("analyze NtcMach");
    eprintln!("[RM8] analyze_binary done");
    ...
    // RM-7: Reconstruct C source from structured IR (all_funcs still borrows built_entries).
    {
        ...
        std::fs::write(r"C:\Users\Administrator\Desktop\rm8_recovered.c", &src)
            .expect("write recovered.c");
        println!("RM-7 wrote rm8_recovered.c ({} bytes, {} funcs)", src.len(), cfuncs.len());
    }
