...
fn ntcmach_path() -> PathBuf {
    PathBuf::from(r"C:\Users\Administrator\Desktop\制版软件\NTCDLLM.DLL")
}
...
fn batch_ntcdllm_dll() {
    ...
    // RM-7: Reconstruct C source from structured IR (all_funcs still borrows built_entries).
    {
        ...
        std::fs::write(r"C:\Users\Administrator\Desktop\ntcdllm_recovered.c", &src)
            .expect("write recovered.c");
        println!("RM-7 wrote ntcdllm_recovered.c ({} bytes, {} funcs)", src.len(), cfuncs.len());
    }
